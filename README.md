...
For the parties and the citizen. Parties can know their situation in the competition. They could adjust their strategy according to the data report. For example, in the report we can analyze the news and tweets to calculate currently which demand is the most welcome, then the party can make the speech concentrate on solving people's demands and persuade people to support them. On the other hand, they also can get their opponent parties' negative information from the report, which may help them to get succeed from the debate of president vote. 
### 4. Technology stack:
...
In this project, we attempt to create a bot that help people successfully buy the hype item likes "the ten Chicago". Those items sold out very fast that they usually been sold out in a few seconds after the released. In the process, we will analyze the frame of website and simulate the process of buying an item. We will accelerate the process of checking out. For example, automatically recognize the product, then add to cart and checkout automatically. The bot should be able to fit every condition, as in common release it is very possible that the website gets crash and the server down. The bot should be able to keep refreshing in certain time frame.(If refresh too fast, the IP will get ban) Moreover, we also need to make bot's movement more be like a human. For instance, supreme website will have a system to detects bot that if client add to cart or checkout too fast would be recognize as using bot. Our bot should be able to circumvent these bot detect tests. 
### 3. Goal of the project (who are you developing the project for)
Benefit for shoe&clothes reseller. Moreover, a successful bot also have a great market. We also can make money at the process of resell bots. Currently, the most popular bot own at least 3k usd value for a copy in the bot market. 
### 4. Technology stack:
